...
export function standardizeLatexMath(raw: string): string {
  ...
  s = s.replace(
    /(\b(số nguyên dương|số nguyên|số tự nhiên|số|ước số|bội số|giá trị|tổng|tích|hiệu|độ dài|chiều dài|vị trí|chỉ số|đỉnh|cạnh|mảng|dãy số|dãy)\s+(của\s+)?)([a-zA-Z])\b/gi,
    (_m, prefix, _của, _space, v) => {
      const idx = mathItems.length;
      mathItems.push(v);
      return `${prefix}___MATH_ITEM_${idx}___`;
    }
  ...
  s = s.replace(
    /\b([a-zA-Z])(\s+(phần tử|số nguyên|số|dòng|cột|ký tự|đỉnh|cạnh|truy vấn|bước|lần|cây|que củi))\b/g,
    (_m, v, suffix) => {
      const idx = mathItems.length;
      mathItems.push(v);
      return `___MATH_ITEM_${idx}___${suffix}`;
    }
  );
